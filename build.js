...
/**
 * HuaFuture Build Script
 * =======================
 * Scans Domestic/ and International/ directories for futures research
 * report HTML files, extracts metadata, and generates index.html
 ...
 *   {品种}-{合约代码}-{报告类型}.html
 *   Example: 螺纹钢-RB2610-深度分析.html
 *   Example: 原油-SC2506-技术分析.html
 *   Example: 黄金-AU2512-基本面研究.html
 */
...
// ─── Config ───────────────────────────────────────────────────────────
const SITE_TITLE = '国仕无双 · 期货研究智库';
const SITE_SUBTITLE = 'HuaFuture Futures Research';
const SITE_DESCRIPTION = '研究覆盖国内期货与国际期货 — 专业深度，套利发现';

// Top-level directories to scan, each maps to a category
const ROOT_DIRS = [
  { dir: 'Domestic',      category: 'Domestic',      rootPath: path.join(__dirname, 'Domestic') },
  { dir: 'International', category: 'International', rootPath: path.join(__dirname, 'International') },
];
...
const CATEGORY_MAP = {
  'Domestic':      { label: '国内', badge: '国内', color: '#d97706' },
  'International': { label: '国际', badge: '国际', color: '#1d4ed8' },
};

// ─── File Scanner ────────────────────────────────────────────────────
function scanHtmlFiles(rootDir, category) {
  const results = [];
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    /* ── Header ── */
    .site-header {
      background: linear-gradient(135deg, #1a0808 0%, #4a1414 50%, #7a2020 100%);
    ...
    <div class="relative z-10 max-w-6xl mx-auto px-6 py-12 sm:py-16">
      <div class="flex items-center gap-3 mb-3">
        <div class="w-10 h-10 rounded-xl overflow-hidden flex-shrink-0 border-2 border-gold/40 shadow-lg shadow-gold/10">
          <img src="favicon.jpg" alt="HuaFuture" class="w-full h-full object-cover" />
        </div>
      ...
      <div class="mt-5 flex flex-wrap gap-3">
        <span class="stat-bubble bg-white/10 text-gray-300 border border-white/10">
          ...
          共 <strong id="total-count">${uniqueTotal}</strong> 份期货研究报告
  ...
  <main class="max-w-6xl mx-auto px-4 sm:px-6 py-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center gap-4 mb-6">
      <div class="flex flex-wrap gap-2">
        ...
        <button class="filter-tab" data-filter="Domestic">🇨🇳 国内</button>
        <button class="filter-tab" data-filter="International">🌐 国际</button>
      </div>
    ...
    <div id="empty-state" class="empty-state hidden">
      ...
      <p class="text-lg font-medium text-gray-400">未找到匹配的期货研究报告</p>
  ...
  <footer class="site-footer">
    <div class="max-w-3xl mx-auto">
      ...
      <p class="mt-3 text-xs text-gray-400">
        © ${new Date().getFullYear()} 国仕无双 · HuaFuture &nbsp;|&nbsp; Powered by Hua Research Platform
      </p>
    </div>
  </footer>
...
function injectBackButton(filePath) {
  ...
  // Skip if already injected (idempotent) — check for either old or new class name
  if (content.indexOf('huafuture-back-btn') !== -1 || content.indexOf('huastock-back-btn') !== -1) return false;

  // Calculate relative path to root (where index.html lives)
  // filePath = "Domestic/xxx.html" → depth=1 → "../index.html"
  // filePath = "International/xxx.html" → depth=1 → "../index.html"
  ...
  const btnHTML =
    `\n<!-- HuaFuture: back-to-home button -->\n` +
    `<a href="${indexPath}" class="huafuture-back-btn" target="_self">\n` +
    ...
    `    background: rgba(74, 20, 20, 0.75) !important;\n` +
    ...
    `  }\n` +
    `  .huafuture-back-btn:hover {\n` +
    `    background: rgba(74, 20, 20, 0.95) !important;\n` +
    `    border-color: rgba(201, 168, 76, 0.5) !important;\n` +
    `    box-shadow: 0 6px 28px rgba(201, 168, 76, 0.18) !important;\n` +
    `    transform: translateY(-2px) !important;\n` +
    `  }\n` +
    `  .huafuture-back-btn svg {\n` +
    `    transition: transform 0.2s ease !important;\n` +
    `  }\n` +
    `  .huafuture-back-btn:hover svg {\n` +
...
// ─── Main ─────────────────────────────────────────────────────────────
function main() {
  console.log('🏭 扫描期货研究报告...');
  const allFiles = [];
  const scanSummaries = [];

  for (const { dir, category, rootPath } of ROOT_DIRS) {
    const found = scanHtmlFiles(rootPath, category);
    allFiles.push(...found);
    scanSummaries.push({ dir, count: found.length, label: CATEGORY_MAP[category]?.label || category });
    console.log(`   ${dir}/ (${CATEGORY_MAP[category]?.label || category}): ${found.length} 份报告`);
  }

  if (allFiles.length === 0) {
    console.log('⚠️  未在 Domestic/ 或 International/ 目录下找到 HTML 报告文件。');
  } else {
    console.log(`📄 共发现 ${allFiles.length} 份期货研究报告`);
  }

  // Extract metadata
  const raw = allFiles.map(f => {
  ...
  // Merge duplicates: same fileName across Domestic + International → single entry
  const grouped = {};
  ...
  console.log('🌐 打开 index.html 即可浏览期货研究智库');
}
